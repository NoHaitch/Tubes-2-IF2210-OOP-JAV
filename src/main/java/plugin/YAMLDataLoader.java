...
public class YAMLDataLoader implements DataLoader {
            ...
            try {
                ...
                if (!card.getName().equals("null")) {
                    ...
                    if (card instanceof Plants) {
                        cardData.put("progress", ((Plants) card).getProgress());
                        cardData.put("protect", ((Plants) card).getProtect());
                        cardData.put("trap", ((Plants) card).getTrap());
                    } else {
                        cardData.put("weight", ((Animal) card).getWeight());
                        cardData.put("protect", ((Animal) card).getProtect());
                        cardData.put("trap", ((Animal) card).getTrap());
                    }
     ...
     *
     * @param file_path relative path to result folder
     * @return Pair, where the first element is Shop and the second element is the current turn
     * @throws Exception file not found, failed to load
     */
    @Override
